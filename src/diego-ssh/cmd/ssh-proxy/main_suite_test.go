...
import (
	...
	"code.cloudfoundry.org/diego-release/diego-ssh/cmd/sshd/testrunner"
...
var _ = SynchronizedBeforeSuite(func() []byte {
	sshProxy, err := gexec.Build("code.cloudfoundry.org/diego-release/diego-ssh/cmd/ssh-proxy", "-race")
	Expect(err).NotTo(HaveOccurred())

	sshd, err := gexec.Build("code.cloudfoundry.org/diego-release/diego-ssh/cmd/sshd", "-race")
	Expect(err).NotTo(HaveOccurred())
	...
	fixturesPath = path.Join(os.Getenv("DIEGO_RELEASE_DIR"), "src/code.cloudfoundry.org/diego-release/diego-ssh/cmd/ssh-proxy/fixtures")
