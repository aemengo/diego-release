...
var _ = SynchronizedBeforeSuite(func() []byte {
	helloPath, err := gexec.Build("code.cloudfoundry.org/diego-release/buildpackapplifecycle/launcher/fixtures/hello")
	Expect(err).NotTo(HaveOccurred())

	launcherPath := buildLauncher()

	getenvPath, err := gexec.Build("code.cloudfoundry.org/diego-release/buildpackapplifecycle/getenv")
	Expect(err).NotTo(HaveOccurred())
