...
import (
	...
	bbsconfig "code.cloudfoundry.org/diego-release/bbs/cmd/bbs/config"
	bbstestrunner "code.cloudfoundry.org/diego-release/bbs/cmd/bbs/testrunner"
	"code.cloudfoundry.org/diego-release/bbs/encryption"
	"code.cloudfoundry.org/diego-release/bbs/test_helpers"
	"code.cloudfoundry.org/diego-release/bbs/test_helpers/sqlrunner"
	...
	"code.cloudfoundry.org/diego-release/route-emitter/cmd/route-emitter/config"
...
var _ = SynchronizedBeforeSuite(func() []byte {
	emitter, err := gexec.Build("code.cloudfoundry.org/diego-release/route-emitter/cmd/route-emitter", "-race")
	Expect(err).NotTo(HaveOccurred())

	bbs, err := gexec.Build("code.cloudfoundry.org/diego-release/bbs/cmd/bbs", "-race")
	Expect(err).NotTo(HaveOccurred())

	locket, err := gexec.Build("code.cloudfoundry.org/diego-release/locket/cmd/locket", "-race")
	Expect(err).NotTo(HaveOccurred())
	...
	basePath := path.Join(os.Getenv("DIEGO_RELEASE_DIR"), "src/code.cloudfoundry.org/diego-release/route-emitter/cmd/route-emitter/fixtures")
